...
const RenovationHeader: React.FC<RenovationHeaderProps> = ({
  ...
  return (
    <div className="flex h-[620px] w-full bg-cover bg-center">
      ...
      <div className="absolute bottom-[-6rem] right-[5rem] flex w-[26rem] flex-col gap-9 rounded-3xl bg-color-dark p-8">
        <h2 className="max-w-64 text-2xl font-normal text-white">{header2}</h2>
        {link_text && (
          <Link href={''} className="relative top-8 text-orange-500 underline">
            {link_text}
          </Link>
        )}
        <div className="button_container mt-16 h-16 w-full">
